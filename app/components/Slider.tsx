...
export default function Slider({ currentIndex, expandedMedia, setExpandedMedia }: SliderProps) {
    ...
    const [isBuffering, setIsBuffering] = useState<boolean[]>([false, false, false, false, false, false]);
            ...
            case 0:
                return (
                    <div className={mediaWrapperClass}>
                        <div
                            className={`${mediaFrameBase} flex items-center justify-center bg-transparent cursor-zoom-in group animate-fade-in`}
                        ...
                        <div className={`${mediaFrameBase} bg-transparent overflow-hidden relative`}>
                            {isBuffering[index] && (
                                <div className="absolute inset-0 flex items-center justify-center z-20 bg-black/20 backdrop-blur-[2px]">
                                    <Loader2 className="w-8 h-8 text-cuivre animate-spin" />
                                </div>
                            )}
                            <video
                                ...
                                muted
                                loop
                                preload={isAdjacent ? "auto" : "metadata"}
                                onWaiting={() => setIsBuffering(prev => {
                                    const c = [...prev]; c[index] = true; return c;
                                })}
                                onPlaying={() => setIsBuffering(prev => {
                                    const c = [...prev]; c[index] = false; return c;
                                })}
                                onCanPlayThrough={() => setIsBuffering(prev => {
                                    const c = [...prev]; c[index] = false; return c;
                                })}
                            >
